...
class CAR_MF_Bias(torch.nn.Module):
    def __init__(self, K, learning_rate, lambda_rate, k_user=30, k_item=20, min_support=0.3, min_confidence=0.5):
        ...
        self.Alpha_embedding = None
        self.Beta_embedding = None
        self.M_tensor = None
        self.N_tensor = None
        self.mu = nn.Parameter(torch.tensor(0.0))
        self.b_u_embedding = None
        self.b_i_embedding = None
    ...
    def _initialize_parameters(self, Y):
        ...
        self.b_u_embedding = torch.nn.Embedding(user_count, 1)
        self.b_i_embedding = torch.nn.Embedding(item_count, 1)
        ...
        nn.init.zeros_(self.b_u_embedding.weight)
        nn.init.zeros_(self.b_i_embedding.weight)
    ...
    def forward(self, user_indices):
        ...
        b_u = self.b_u_embedding(user_indices)
        b_i = self.b_i_embedding.weight.T

        prediction = self.mu + b_u + b_i + (p_u @ q_i.T) + (alpha_u * m_ui) + (beta_u * n_ui)
        return prediction
    ...
    def fit(self, Y, epochs, batch_size=32):
        ...
        params_without_decay = [
            self.Alpha_embedding.weight,
            self.Beta_embedding.weight,
            self.b_u_embedding.weight,
            self.b_i_embedding.weight,
            self.mu
        ]
    ...
    def predict(self):
        ...
        with torch.no_grad():
            ...
            parameters = {
                ...
                'bu': self.b_u_embedding.weight.detach().cpu().numpy(),
                'bi': self.b_i_embedding.weight.detach().cpu().numpy()
            }
            return {
