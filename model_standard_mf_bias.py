...
class Standard_MF_Bias(torch.nn.Module):
    ...
    def __init__(self, user_count, item_count, K, learning_rate, lambda_rate):
    ...
    def forward(self, user_indices):
        ...
        item_factors = self.Q_embedding.weight
        
        user_b = self.user_bias(user_indices)
        item_b = self.item_bias.weight
        
        y_prediction = (user_factors @ item_factors.T) + user_b + item_b.T + self.global_bias
    ...
    def fit(self, Y, epochs, batch_size=32):
        ...
        num_observed_ratings = torch.sum(observed_mask).item()
        ...
        for epoch in range(epochs):
            ...
            for user_indices, Y_batch in dataloader:
                ...
                loss = self.loss_fn(y_pred_observed, y_true_observed)
                ...
                total_loss_y += loss.item()

            if (epoch + 1) % 5 == 0 or epoch == epochs - 1:
                rmse_y = np.sqrt(total_loss_y / num_observed_ratings)
                print(f"Epoch {epoch+1}/{epochs}, Train RMSE (Y): {rmse_y:.4f}")
